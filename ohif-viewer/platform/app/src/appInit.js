...
async function appInit(appConfigOrFunc, defaultExtensions, defaultModes) {
  ...
  // Keep the original uncertainty mode module for creating C0/C1 variants.
  let uncertaintyModeModule = null;
  ...
  for (let i = 0; i < loadedModes.length; i++) {
    ...
    // Remember the original module so we can instantiate C0/C1 variants.
    if (id === '@thesis/mode-uncertainty-review') {
      uncertaintyModeModule = mode;
    }
  ...
  // Derive C0 (Manual) and C1 (MONAILabel) mode buttons from the uncertainty mode.
  // These share the same extension code but default to different conditions.
  if (uncertaintyModeModule && typeof uncertaintyModeModule.modeFactory === 'function') {
    const factory = uncertaintyModeModule.modeFactory;

    const c0Variant = factory({
      modeConfiguration: { defaultCondition: 'C0', displayName: 'Manual' },
    });
    if (c0Variant && c0Variant.id && !modesById.has(c0Variant.id)) {
      modesById.add(c0Variant.id);
      appConfig.loadedModes.push(c0Variant);
    }

    const c1Variant = factory({
      modeConfiguration: { defaultCondition: 'C1', displayName: 'MONAILabel' },
    });
    if (c1Variant && c1Variant.id && !modesById.has(c1Variant.id)) {
      modesById.add(c1Variant.id);
      appConfig.loadedModes.push(c1Variant);
    }
  }
