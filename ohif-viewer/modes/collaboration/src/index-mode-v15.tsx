        ...
        if (volume?.scalarData) {
          ...
          if (uint8Array.length < 20 * 1024 * 1024) {
            ...
            try {
              ...
              const viewportService = (window as any).servicesManager?.services?.cornerstoneViewportService;
              console.log('[Mode] 🎥 viewportService exists:', !!viewportService);
              const viewportIds = viewportService?.getViewportIds?.() || [];
              console.log('[Mode] 🎥 viewportIds:', viewportIds);
              if (viewportIds.length > 0) {
                const renderingEngine = viewportService?.getRenderingEngine?.();
                console.log('[Mode] 🎥 renderingEngine exists:', !!renderingEngine);
                const viewport = renderingEngine?.getViewport(viewportIds[0]);
                console.log('[Mode] 🎥 viewport exists:', !!viewport, 'type:', viewport?.type);
                if (viewport) {
                ...
                console.log('[Mode] ⚠️ No viewportIds found');
              }
            } catch (camError) {
      ...
      try {
        ...
        if (segmentationService) {
          ...
          if (events?.SEGMENTATION_DATA_MODIFIED) {
            const sub = segmentationService.subscribe(events.SEGMENTATION_DATA_MODIFIED, (evt) => {
              console.log('🔄 Segmentation data modified:', evt);
              ...
              console.log('🔍 [Mode] Socket check:', !!socket, 'SessionId:', sessionId);
              
              if (socket && sessionId) {
                const segmentationId = evt?.segmentationId || evt?.segmentation?.id || evt?.segmentation?.segmentationId;
                ...
                if (segmentationId) {
                  ...
                  setTimeout(() => {
                    try {
                      ...
                      if (fullSeg) {
                        const syncData = serializeSegmentationForSync(fullSeg);
                        
                        // Only broadcast if we have labelmap data
                        if (syncData && syncData.labelmapData) {
                          // Check if this is the first broadcast for this segmentation
                          const isFirstBroadcast = !broadcastedSegmentations.has(segmentationId);
                          
                          if (isFirstBroadcast) {
                            // First time - send as 'add'
                            socket.emit('segmentation:add', {
                              sessionId: sessionId,
                              segmentation: syncData,
                            });
                            broadcastedSegmentations.add(segmentationId);
                            console.log('📤 [Mode] Initial segmentation broadcast sent (add), hasLabelmapData=true');
                          } else {
                            // Subsequent updates
                            socket.emit('segmentation:update', {
                              sessionId: sessionId,
                              segmentation: syncData,
                            });
                            console.log('📤 [Mode] Segmentation update sent');
                          }
                        } else {
                          console.log('⏳ [Mode] Labelmap data not ready yet, skipping broadcast');
                        }
                      }
                    } catch (e) {
                      console.error('❌ [Mode] Failed to broadcast segmentation:', e);
                    }
                  }, 100); // Short delay - data should already be there at this point
                } else {
                  console.log('⚠️ [Mode] No segmentationId extracted from event');
                }
              } else {
                console.log('⚠️ [Mode] Not in collaboration session, skipping broadcast');
              }
            });
            modeCleanupFunctions.push(() => sub?.unsubscribe?.());
          }
        ...
        console.warn('Segmentation setup warning:', e);
      }
