...
export default class MonaiLabelClient {
  constructor(server_url) {
    ...
    // Fix: Ensure the server_url ends with a slash to prevent URL constructor from stripping path segments
    if (!this.server_url.pathname.endsWith('/')) {
      this.server_url.pathname += '/';
    }
  }
  ...
  async is_train_running() {
    ...
    // Fix: Add robust check for response existence and data structure to avoid TypeError on 404
    return (
      response && 
      response.status === 200 && 
      response.data && 
