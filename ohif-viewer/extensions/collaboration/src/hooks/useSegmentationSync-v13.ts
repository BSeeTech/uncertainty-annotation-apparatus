                  ...
                  try {
                    ...
                    if (volumeViewport && 'setVolumes' in volumeViewport) {
                      ...
                      try {
                        ...
                        // Navigate to first slice (slice index 0) using multiple approaches
                        ...
                        console.log('[SegmentationSync] Has setSliceIndex:', !!volumeViewport.setSliceIndex);
                        console.log('[SegmentationSync] Has scroll:', !!volumeViewport.scroll);
                        
                        let navigated = false;
                        
                        // Method 1: setSliceIndex
                        if (volumeViewport.setSliceIndex) {
                          try {
                            volumeViewport.setSliceIndex(0);
                            console.log('[SegmentationSync] ✅ Used setSliceIndex(0)');
                            navigated = true;
                          } catch (e) {
                            console.log('[SegmentationSync] setSliceIndex failed:', e);
                          }
                        }
                        
                        // Method 2: scroll from current position
                        if (!navigated && volumeViewport.scroll) {
                          try {
                            const numSlices = refVolume?.dimensions?.[2] || 38;
                            // Scroll to the beginning (negative scroll = toward first slice)
                            volumeViewport.scroll(-numSlices);
                            console.log('[SegmentationSync] ✅ Used scroll(-' + numSlices + ')');
                            navigated = true;
                          } catch (e) {
                            console.log('[SegmentationSync] scroll failed:', e);
                          }
                        }
                        
                        // Method 3: Camera focal point manipulation
                        if (!navigated && refVolume?.origin && camera) {
                          try {
                            const firstSliceZ = refVolume.origin[2];
                            const newFocalPoint = [camera.focalPoint[0], camera.focalPoint[1], firstSliceZ];
                            const deltaZ = firstSliceZ - camera.focalPoint[2];
                            const newPosition = [camera.position[0], camera.position[1], camera.position[2] + deltaZ];
                            ...
                            console.log('[SegmentationSync] ✅ Used camera focal point (Z=' + firstSliceZ + ')');
                            navigated = true;
                          } catch (e) {
                            console.log('[SegmentationSync] Camera manipulation failed:', e);
                          }
                        }
                        
                        if (!navigated) {
                          console.log('[SegmentationSync] ⚠️ Could not navigate to first slice');
                        }
