    ...
    const handleViewportSync = (data: any) => {
      ...
      try {
        ...
        if (vp && vp.type === 'orthographic') {
          ...
          if (currentCamera && data.camera.focalPoint) {
            console.log('[SegmentationSync] 📥 Received viewport:sync, applying camera');
            
            // Apply full camera state from presenter
            const newCamera = {
              ...currentCamera,
              focalPoint: data.camera.focalPoint,
              position: data.camera.position,
              parallelScale: data.camera.parallelScale || currentCamera.parallelScale,
            };
