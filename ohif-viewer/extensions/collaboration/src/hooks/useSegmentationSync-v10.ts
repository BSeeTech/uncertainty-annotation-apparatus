          ...
          try {
            ...
            if (!segmentationVolume) {
              ...
              if (cornerstone.volumeLoader?.createAndCacheDerivedSegmentationVolume) {
                ...
                // CRITICAL: Override the direction matrix to identity
                // This prevents csTools from applying its own orientation correction,
                // so the segmentation will be displayed with the same flip as the base image
                if (segmentationVolume?.direction) {
                  const identityDirection = new Float32Array([1, 0, 0, 0, 1, 0, 0, 0, 1]);
                  console.log('[SegmentationSync] Original seg direction:', Array.from(segmentationVolume.direction));
                  segmentationVolume.direction.set(identityDirection);
                  console.log('[SegmentationSync] Set seg direction to identity:', Array.from(segmentationVolume.direction));
                }
              } else {
            ...
            if (segmentationVolume?.scalarData) {
              ...
              if (finalSegVolume?.scalarData) {
                ...
                // Apply Y+X flip to match the volume viewport's rendering of the base image
                // The base image appears flipped in the volume viewport, so we flip the
                // segmentation to match it. Then we'll try to flip the viewport to correct both.
                if (dims && dims.length === 3) {
                  console.log('[SegmentationSync] 🔄 Applying Y+X flip transformation');
                  dataToApply = flipLabelmapYX(labelmapDataToCopy, dims);
                } else {
                  console.log('[SegmentationSync] ⚠️ No dimensions available for flip');
                }
