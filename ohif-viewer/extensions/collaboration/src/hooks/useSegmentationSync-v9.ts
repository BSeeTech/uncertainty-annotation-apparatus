          ...
          try {
                    ...
                    if (volumeViewport && 'setVolumes' in volumeViewport) {
                      ...
                      try {
                        if (volumeViewport.setProperties) {
                          ...
                          console.log('[SegmentationSync] Applied viewport flip (H+V)');
                        }
                        
                        // Reset and render to apply changes
            ...
            if (segmentationVolume?.scalarData) {
              ...
              if (finalSegVolume?.scalarData) {
                ...
                // DISABLED: Let segmentation match the base image orientation
                // The camera will be fixed separately to un-flip both
                if (dims && dims.length === 3) {
                  console.log('[SegmentationSync] ⚠️ NOT applying flip - letting segmentation match base image orientation');
                  // dataToApply = flipLabelmapYX(labelmapDataToCopy, dims);  // DISABLED
                } else {
              ...
              setTimeout(() => {
                try {
                  ...
                  if (vp && vp.type === 'orthographic') {
                    console.log('[SegmentationSync] 🔧 Attempting post-render camera fix...');
                    
                    // Get current camera
                    const camera = vp.getCamera?.();
                    console.log('[SegmentationSync] Current camera:', JSON.stringify(camera));
                    
                    // Try flip via setProperties
                    if (vp.setProperties) {
                      vp.setProperties({ flipHorizontal: true, flipVertical: true });
                      console.log('[SegmentationSync] Applied flip properties');
                    }
                    
                    // Navigate to first slice
                    if (vp.setSliceIndex) {
                      vp.setSliceIndex(0);
                      console.log('[SegmentationSync] Set slice to 0');
                    }
                    
                    vp.render?.();
                  }
                } catch (e) {
                  console.log('[SegmentationSync] Post-render camera fix failed:', e);
                }
              }, 200);
