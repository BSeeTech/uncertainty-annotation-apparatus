          ...
          try {
            ...
            if (segmentationVolume?.scalarData) {
              ...
              if (finalSegVolume?.scalarData) {
                ...
                // Try X flip only - if base image is correct and overlay was H-flipped with Y+X,
                // removing Y (applying only X) should fix it
                if (dims && dims.length === 3) {
                  console.log('[SegmentationSync] 🔄 Applying X flip only (horizontal)');
