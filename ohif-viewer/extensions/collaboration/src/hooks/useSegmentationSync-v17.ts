  ...
  useEffect(() => {
    ...
    const handleViewportSync = (data: any) => {
      ...
      try {
        ...
        if (viewportState?.camera && isVolumeViewport) {
          // Volume viewport - apply camera state (Presenter is also volume)
          const currentCamera = vp.getCamera?.();
          if (currentCamera && viewportState.camera.focalPoint) {
            console.log('[SegmentationSync] 📥 Received viewport:sync (volume→volume), Z:', 
              viewportState.camera.focalPoint[2]?.toFixed?.(1));
        ...
        } else if (viewportState?.imageIndex !== undefined && isVolumeViewport) {
          // MISMATCH: Presenter is stack, Follower is volume
          // Convert image index to camera Z position
          const currentCamera = vp.getCamera?.();
          if (!currentCamera) return;
          
          // Get volume to calculate Z position from image index
          ...
          // Fallback: search cache for non-segmentation volume
          if (!refVolume) {
            const volumes = cornerstone.cache?.getVolumes?.() || [];
            for (const vol of volumes) {
          ...
          if (refVolume && refVolume.origin && refVolume.spacing) {
            ...
            if (Math.abs(targetZ - currentZ) > 0.1) {
              console.log('[SegmentationSync] 📥 Received viewport:sync (stack→volume), index:', 
                targetIndex, '→ Z:', targetZ.toFixed(1));
              
              const deltaZ = targetZ - currentZ;
              const newCamera = {
                ...currentCamera,
                focalPoint: [currentCamera.focalPoint[0], currentCamera.focalPoint[1], targetZ],
                position: [currentCamera.position[0], currentCamera.position[1], currentCamera.position[2] + deltaZ],
              };
          ...
          // Stack viewport - apply image index directly (both are stack)
          const currentIndex = vp.getCurrentImageIdIndex?.() ?? -1;
          if (currentIndex !== viewportState.imageIndex) {
            console.log('[SegmentationSync] 📥 Received viewport:sync (stack→stack), index:', viewportState.imageIndex);
            vp.setImageIdIndex?.(viewportState.imageIndex);
            vp.render?.();
          }
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const broadcastViewportState = () => {
      try {
        ...
        if (isVolumeViewport) {
          ...
          const currentIndex = vp.getCurrentImageIdIndex?.() ?? -1;
          if (currentIndex === lastImageIndex) return;
          
          lastImageIndex = currentIndex;
          
          console.log('[SegmentationSync] 🎥 Broadcasting image index:', currentIndex);
    ...
    const attachListeners = () => {
      try {
        ...
        const vp = viewports[0];
        if (vp?.element && vp.element !== attachedElement) {
    ...
    // Attach listeners initially and re-check periodically (in case viewport changes)
    attachListeners();
    const listenerCheckInterval = setInterval(attachListeners, 2000);
