          ...
          try {
                ...
                if (element && renderingEngine) {
                  try {
                    ...
                    const viewportInput = {
                      ...
                      element,
                      defaultOptions: {
                        orientation: 'acquisition' as any,  // Use 'acquisition' to match DICOM orientation
                    ...
                    if (volumeViewport && 'setVolumes' in volumeViewport) {
                      ...
                      // Try to restore camera orientation to match DICOM data
                      try {
                        // Get the volume to access its direction matrix
                        const refVolume = cornerstone.cache.getVolume(referenceVolumeId);
                        if (refVolume?.direction) {
                          const dir = refVolume.direction;
                          console.log('[SegmentationSync] Volume direction matrix:', Array.from(dir));
                          
                          // Set camera to view from the correct angle
                          // For direction [-1, 0, 0, 0, -1, 0, 0, 0, 1], we need to flip the view
                          const camera = volumeViewport.getCamera?.();
                          if (camera) {
                            // Flip the view vectors to match the expected orientation
                            const newCamera = {
                              ...camera,
                              viewUp: [0, -1, 0],  // Flip Y view direction
                              viewPlaneNormal: [0, 0, -1],  // Look from opposite Z direction
                            };
                            volumeViewport.setCamera?.(newCamera);
                            console.log('[SegmentationSync] Camera orientation adjusted');
                          }
                        }
                        
                        // Navigate to the correct slice
                        if (volumeViewport.setSlabThickness && currentSliceIndex > 0) {
                          // For volume viewports, we use scroll to navigate
                          const numSlices = refVolume?.dimensions?.[2] || 38;
                          const sliceOffset = currentSliceIndex - Math.floor(numSlices / 2);
                          volumeViewport.scroll?.(sliceOffset);
                          console.log('[SegmentationSync] Scrolled to slice offset:', sliceOffset);
                        }
                      } catch (cameraError) {
                        console.log('[SegmentationSync] Camera adjustment failed:', cameraError);
                      }
                    }
            ...
            if (segmentationVolume?.scalarData) {
              ...
              if (finalSegVolume?.scalarData) {
                ...
                // Apply Y+X flip to fix orientation mismatch (vertical + horizontal)
                if (dims && dims.length === 3) {
                  console.log('[SegmentationSync] 🔄 Applying Y+X flip transformation');
                  dataToApply = flipLabelmapYX(labelmapDataToCopy, dims);
                } else {
