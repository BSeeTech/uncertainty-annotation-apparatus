...
/**
 * Flips labelmap data along the Z-axis (slice order reversal)
 * This is the most common orientation mismatch in DICOM viewing
 */
function flipLabelmapZ(data: Uint8Array, dims: number[]): Uint8Array {
  const [width, height, depth] = dims;
  const sliceSize = width * height;
  const result = new Uint8Array(data.length);
  
  for (let z = 0; z < depth; z++) {
    const srcSliceStart = (depth - 1 - z) * sliceSize;
    const dstSliceStart = z * sliceSize;
    result.set(data.subarray(srcSliceStart, srcSliceStart + sliceSize), dstSliceStart);
  }
          ...
          try {
            ...
            if (segmentationVolume?.scalarData) {
              ...
              if (finalSegVolume?.scalarData) {
                // Apply Z-flip to fix orientation mismatch (most common DICOM issue)
                const dims = segmentationData.labelmapDimensions;
                let dataToApply = labelmapDataToCopy;
                if (dims && dims.length === 3) {
                  console.log('[SegmentationSync] Applying Z-flip transformation');
                  dataToApply = flipLabelmapZ(labelmapDataToCopy, dims);
                }
                ...
                console.log('[SegmentationSync] Copied labelmap data AFTER registration (with Z-flip), length:', copyLength);
                
                // Verify some data was copied
                let nonZeroCount = 0;
                for (let i = 0; i < Math.min(copyLength, 100000); i++) {
                  if (targetArray[i] !== 0) nonZeroCount++;
                }
                console.log('[SegmentationSync] Non-zero voxels in first 100K:', nonZeroCount);
              } else {
