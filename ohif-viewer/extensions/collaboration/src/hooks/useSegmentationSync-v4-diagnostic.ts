          ...
          try {
            ...
            if (segmentationVolume?.scalarData) {
              ...
              if (finalSegVolume?.scalarData) {
                ...
                console.log('[SegmentationSync] 📊 Direction from Presenter:', segmentationData.direction);
                console.log('[SegmentationSync] 📊 Local direction:', finalSegVolume.direction ? Array.from(finalSegVolume.direction) : 'N/A');
                
                // TEMPORARILY DISABLED: flip transformation for testing
                // Uncomment ONE of these to test different flip combinations:
                
                // Option 1: No flip (direct copy)
                console.log('[SegmentationSync] ⚠️ Using NO FLIP (direct copy for testing)');
                // dataToApply = labelmapDataToCopy;
                
                // Option 2: Y+X flip (vertical + horizontal)
                // if (dims && dims.length === 3) {
                //   console.log('[SegmentationSync] Applying Y+X flip');
                //   dataToApply = flipLabelmapYX(labelmapDataToCopy, dims);
                // }
                
                // Option 3: Y flip only (vertical)
                // if (dims && dims.length === 3) {
                //   console.log('[SegmentationSync] Applying Y flip only');
                //   dataToApply = flipLabelmapY(labelmapDataToCopy, dims);
                // }
                
                // Option 4: X flip only (horizontal)
                // if (dims && dims.length === 3) {
                //   console.log('[SegmentationSync] Applying X flip only');
                //   dataToApply = flipLabelmapX(labelmapDataToCopy, dims);
                // }
