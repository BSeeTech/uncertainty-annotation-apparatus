...
export function useSegmentationSync({
  ...
  useEffect(() => {
    ...
    // Handle viewport sync updates for volume viewports
    // This is needed because volume viewports use camera position, not image index
    // Server emits 'viewport:sync' when presenter sends 'viewport:update'
    const handleViewportSync = (data: any) => {
      // Extract camera from viewportState wrapper (server structure)
      const viewportState = data?.viewportState || data;
      const camera = viewportState?.camera;
      
      if (!camera) {
        // Might be a stack viewport sync with imageIndex instead
        return;
      }
      ...
      try {
        ...
        if (vp && vp.type === 'orthographic') {
          ...
          if (currentCamera && camera.focalPoint) {
            console.log('[SegmentationSync] 📥 Received viewport:sync, applying camera Z:', camera.focalPoint[2]?.toFixed?.(1));
            
            // Apply full camera state from presenter
            const newCamera = {
              ...currentCamera,
              focalPoint: camera.focalPoint,
              position: camera.position,
              parallelScale: camera.parallelScale || currentCamera.parallelScale,
            };
        ...
        console.log('[SegmentationSync] Viewport sync handler error:', e);
      }
    };
  ...
  useEffect(() => {
    ...
    console.log('[SegmentationSync] 🎥 Setting up volume viewport camera sync for presenter');
    
    let lastCameraZ = 0;
    let broadcastTimeout: ReturnType<typeof setTimeout> | null = null;
    
    const broadcastCameraPosition = () => {
      try {
        const vpService = getCornerstoneViewportService();
        const viewportId = vpService?.getViewportIds?.()?.[0];
        const re = vpService?.getRenderingEngine?.();
        ...
        if (!vp || vp.type !== 'orthographic') return;
        
        const camera = vp.getCamera?.();
        if (!camera) return;
        
        // Only broadcast if Z position changed significantly
        const currentZ = camera.focalPoint?.[2] || 0;
        if (Math.abs(currentZ - lastCameraZ) < 0.1) return;
        
        lastCameraZ = currentZ;
        
        console.log('[SegmentationSync] 🎥 Broadcasting camera position, Z:', currentZ.toFixed(1));
    ...
    // Set up polling for camera changes (works even without direct event access)
    const pollInterval = setInterval(throttledBroadcast, 200);
    
    // Also try to listen to cornerstone events
    const handleCameraModified = () => throttledBroadcast();
    ...
    try {
      ...
      if (vp?.element) {
        vp.element.addEventListener('cornerstonecameramodified', handleCameraModified);
        vp.element.addEventListener('CORNERSTONE_CAMERA_MODIFIED', handleCameraModified);
        console.log('[SegmentationSync] 🎥 Attached camera event listeners');
      }
    } catch (e) {
      console.log('[SegmentationSync] Could not attach camera listeners:', e);
    }
    ...
    return () => {
      ...
      try {
        ...
        if (vp?.element) {
          vp.element.removeEventListener('cornerstonecameramodified', handleCameraModified);
          vp.element.removeEventListener('CORNERSTONE_CAMERA_MODIFIED', handleCameraModified);
        }
      } catch (e) {}
    };
  }, [isEnabled, collaborationService, sessionId, role, getCornerstoneViewportService]);
