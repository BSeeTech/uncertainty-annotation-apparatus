...
class SegmentationService extends PubSubService {
  ...
  public destroy = () => {
    ...
    // Cornerstone3D V3: fully reset segmentation state to prevent stale data
    // from interfering with the next mode entry.  removeAllSegmentations
    // cleans up cornerstones's internal segmentation registry and
    // removeAllSegmentationRepresentations cleans up per-viewport mappings.
    try {
      cstSegmentation.removeAllSegmentationRepresentations();
    } catch (e) {
      console.warn('SegmentationService.destroy: removeAllSegmentationRepresentations', e);
    }
    try {
      cstSegmentation.removeAllSegmentations();
    } catch (e) {
      console.warn('SegmentationService.destroy: removeAllSegmentations', e);
    }
  ...
  public removeSegment(segmentationId: string, segmentIndex: number): void {
    ...
    if (!labelmapVolume) {
      console.warn(
        `SegmentationService::removeSegment: labelmap volume not found for segmentation ${segmentationId}`
      );
      // Still broadcast the update event so the UI refreshes
      this._broadcastEvent(this.EVENTS.SEGMENTATION_UPDATED, {
        segmentation,
      });
      return;
    ...
    // Cornerstone3D V3: scalarData accessed through voxelManager.
    ...
    // Get a complete scalar data array (copy), modify it, then write it back.
    ...
    // Write the modified scalar data back through the voxelManager.
  ...
  public async createSegmentationForSEGDisplaySet(
    ...
    // Cornerstone3D V3: scalarData accessed through voxelManager.
    const derivedVolumeVoxelManager = derivedVolume.voxelManager;
    const derivedVolumeScalarData = derivedVolumeVoxelManager.getCompleteScalarDataArray();

    const segmentsInfo = segDisplaySet.segMetadata.data;
    derivedVolumeScalarData.set(new Uint8Array(labelmapBufferArray[0]));
    derivedVolumeVoxelManager.setCompleteScalarDataArray(derivedVolumeScalarData);
  ...
  public calculateCentroids = (
    ...
    if (!volume) {
      console.warn(
        `SegmentationService::calculateCentroids: labelmap volume not found for segmentation ${segmentationId}`
      );
      return new Map();
    }
  ...
  private setCentroids = (
    ...
    const segmentation = this.getSegmentation(segmentationId);
    const imageData = this.getLabelmapVolume(segmentationId)?.imageData;
    ...
    for (const [segmentIndex, centroid] of centroids) {
      ...
      if (!world || world.length === 0) {
        world = imageData?.indexToWorld(centroid.image) || centroid.image;
      }
  ...
  public jumpToSegmentCenter(
    ...
    toolGroups.forEach(toolGroup => {
      ...
      for (const { viewportId, renderingEngineId } of viewportsInfo) {
        ...
        csUtilities.jumpToWorld(viewport, world);
      }
  ...
  public addSegmentationRepresentationToToolGroup = async (
    ...
    // Cornerstone3D V3+: addSegmentationRepresentations is per-viewport, not per-toolGroup,
    // and does not return UIDs.  Get the viewportIds from the toolGroup and add
    // the representation to each viewport.
    const { toolGroupService } = this.servicesManager.services;
  ...
  public removeSegmentationRepresentationFromToolGroup(
    ...
    cstSegmentation.removeSegmentationRepresentations(toolGroupId, uids);
  }
  ...
  public getConfiguration = (toolGroupId?: string): SegmentationConfig => {
    ...
    const renderInactiveSegmentations = false;

    const {
      renderOutline,
      outlineWidth: outlineWidthActive,
      renderFill,
      fillAlpha,
      fillAlphaInactive,
      outlineOpacity,
  ...
  public setConfiguration = (configuration: SegmentationConfig): void => {
    ...
    if (renderInactiveSegmentations !== undefined) {
      // Use the dedicated API for renderInactiveSegmentations (viewport-level setting)
      const { cornerstoneViewportService } = this.servicesManager.services;
      const viewportIds = cornerstoneViewportService.getViewportIds();
      viewportIds.forEach(viewportId => {
        cstSegmentation.config.style.setRenderInactiveSegmentations(
          viewportId,
          renderInactiveSegmentations
        );
      });
  ...
  private _setActiveSegmentationForToolGroup(
    ...
    // Cornerstone3D V3+: setActiveSegmentation is per-viewport (viewportId, segmentationId),
    // not per-toolGroup (toolGroupId, representationUID).  Get viewportIds from the toolGroup.
    const { toolGroupService } = this.servicesManager.services;
    const toolGroup = toolGroupService.getToolGroup(toolGroupId);
    const viewportIds = toolGroup ? toolGroup.getViewportIds() : [];
  ...
  private _setSegmentationConfig = (property, value) => {
    ...
    cstSegmentation.config.style.setStyle({ type: typeToUse }, config);
  ...
  private _onSegmentationModifiedFromSource = evt => {
    ...
    const { activeSegmentIndex, cachedStats, segmentsLocked, label } = segmentationState;
    ...
    const type =
      ...
      (segmentationState.representationData?.Labelmap
        ...
        : segmentationState.representationData?.Contour
          ? CONTOUR
          : Object.keys(segmentationState.representationData ?? {})[0]);
  ...
  private _removeSegmentationFromCornerstone(segmentationId: string) {
    ...
    // Cornerstone3D V3: getViewportIdsWithSegmentation replaces getToolGroupIdsWithSegmentation
    const viewportIds = segmentationState.getViewportIdsWithSegmentation(segmentationId);

    const { toolGroupService } = this.servicesManager.services;

    viewportIds.forEach(viewportId => {
      // Track the toolGroupId for the caller
      const toolGroup = toolGroupService.getToolGroupForViewport(viewportId);
      if (toolGroup) {
        updatedToolGroupIds.add(toolGroup.id);
      }

      // V3: removeSegmentationRepresentations takes viewportId + specifier object
