...
import { utilities as csToolsUtils, Enums as csToolsEnums, segmentation as cstSegmentation } from '@cornerstonejs/tools';
...
class CornerstoneViewportService extends PubSubService implements IViewportService {
  ...
  public getRenderingEngine() {
    ...
    // Create a new rendering engine if none exists or the cached one was destroyed
  ...
  public destroy() {
    ...
    // Clear CornerstoneCacheService internal state to prevent stale
    // stackImageIds / volumeImageIds from the previous study session
    const { cornerstoneCacheService } = this.servicesManager.services as any;
    if (cornerstoneCacheService?.destroy) {
   ...
   */
  public disableElement(viewportId: string, elementRef?: HTMLDivElement): void {
    const viewportInfo = this.viewportsById.get(viewportId);

    // Viewport ids are reused while navigating between studies.  A cleanup
    // from the previous route must not disable the new study's viewport.
    if (elementRef && viewportInfo?.getElement() !== elementRef) {
      return;
    }
  ...
  public getPositionPresentation(viewportId: string): PositionPresentation {
    ...
    // For volume viewports, getCurrentImageIdIndex() throws when no volume is
    // loaded yet (e.g., during setViewportData before _setDisplaySets finishes).
    // Check if the viewport has actors loaded before trying to get the image index.
    let initialImageIndex = 0;
    if (csViewport instanceof BaseVolumeViewport) {
      const defaultActor = csViewport.getDefaultActor?.();
      if (defaultActor?.uid) {
        initialImageIndex = csViewport.getCurrentImageIdIndex() || 0;
      }
    } else {
      initialImageIndex = csViewport.getCurrentImageIdIndex() || 0;
    }
  ...
  public setViewportData(
    ...
    displaySetPromise.then(() => {
      ...
      console.warn(
        `CornerstoneViewportService::setViewportData: failed to set data for viewport ${viewportId}`,
        e
      );
    });
  }
  ...
  public getCornerstoneViewport(viewportId: string): Types.IViewport | null {
    ...
    try {
      ...
      console.warn(
        `CornerstoneViewportService::getCornerstoneViewport: viewport ${viewportId} not found in rendering engine`,
        e
      );
      return null;
    }
  ...
  async _setVolumeViewport(
    ...
    if (volumesNotLoaded.length) {
      ...
      // Must AWAIT volume loads before calling setVolumesForViewport!
      // Without this, viewport.setVolumes() will throw because the volume
      // isn't in cache yet ("imageVolume does not exist").
      try {
        await Promise.all(
          volumesNotLoaded.map(async volume => {
        ...
        console.warn(
          'CornerstoneViewportService::_setVolumeViewport: volume load failed',
          e
        );
      }
    }
  ...
  public async setVolumesForViewport(viewport, volumeInputArray, presentations) {
    ...
    // Cornerstone3D V3: When switching to a new study, remove previous study's
    // segmentation representations from each viewport so that stale labelmap
    // volumes (different FrameOfReference) do not interfere with the new volume's
    // rendering.  The call below is safe when there are no representations.
    const toolGroup = toolGroupService.getToolGroupForViewport(viewport.id);
    if (toolGroup) {
      const viewportIds = toolGroup.getViewportIds();
      for (const vpId of viewportIds) {
        const segReps =
          cstSegmentation.state.getSegmentationRepresentations(vpId);
        if (segReps?.length) {
          for (const rep of segReps) {
            cstSegmentation.removeSegmentationRepresentations(vpId, {
    ...
    // Wrap setVolumes in try/catch: if the volume isn't in cache yet
    // (e.g., after cache.purgeCache() during a study switch, or a race
    // condition in volume loading), Cornerstone3D throws:
    // "imageVolume with id: ${volumeId} does not exist".
    // Without this guard, the error becomes an unhandled promise rejection
    // and the viewport stays blank forever (canvas was already cleared by
    // enableElement).
    try {
      await viewport.setVolumes(volumeInputArray);
    } catch (e) {
      console.warn(
        'CornerstoneViewportService::setVolumesForViewport: setVolumes failed, retrying with load',
        e
      );
      // Retry: pre-load each volume and then call setVolumes again.
      for (const input of volumeInputArray) {
        try {
          const vol = cache.getVolume(input.volumeId);
          if (vol && !vol.loadStatus.loaded) {
            await vol.load();
          }
        } catch (loadErr) {
          console.warn(
            'CornerstoneViewportService::setVolumesForViewport: retry load failed for',
            input.volumeId,
            loadErr
          );
        }
      }
      // Second attempt after ensuring volumes are loaded
      await viewport.setVolumes(volumeInputArray);
    }
    ...
    if (imageIndex !== undefined) {
      await csUtils.jumpToSlice(viewport.element, {
        imageIndex,
      });
    }
  ...
  public updateViewport(viewportId: string, viewportData, keepCamera = false) {
    ...
    displaySetPromise.then(() => {
      ...
      console.warn(
        `CornerstoneViewportService::updateViewport: failed for viewport ${viewportId}`,
        e
      );
    });
  }
