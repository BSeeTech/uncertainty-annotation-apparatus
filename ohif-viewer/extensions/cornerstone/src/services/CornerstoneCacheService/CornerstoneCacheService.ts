...
class CornerstoneCacheService {
  ...
  private _shouldRenderSegmentation(displaySets) {
    ...
    // Check 1: Existing segmentations reference these display sets
    ...
    // Check 2: Display sets are reconstructable (can support future segmentations).
    // In segmentation-supporting modes (detected by Brush tools), start with a
    // volume viewport so users can add segmentations without a stack→volume
    // conversion that can cause blank viewports.
    const brushToolGroups = ToolGroupManager.getToolGroupsWithToolName('CircularBrush');
    if (brushToolGroups.length > 0) {
      for (const displaySet of displaySets) {
