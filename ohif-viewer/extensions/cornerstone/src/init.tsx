...
import createResetCrosshairsOnCameraReset from './utils/resetCrosshairsOnCameraReset';
...
export default async function init({
  ...
  // Cornerstone3D v2+ handles SharedArrayBuffer configuration automatically.
  // The removed setUseSharedArrayBuffer / SharedArrayBufferModes API is not used.
  // SharedArrayBuffer handling stays inside Cornerstone3D V2 runtime setup.
  const value = appConfig.useSharedArrayBuffer;
  let sharedArrayBufferDisabled = false;

  if (value === 'FALSE' || value === false) {
    sharedArrayBufferDisabled = true;
    // In v2, the buffer mode is handled implicitly; we just note the config.
  }
  ...
  // Cornerstone3D v2 uses segmentation.config.style.setStyle with a
  // specifier that includes the representation type.
  ...
  const resetCrosshairsOnCameraReset = createResetCrosshairsOnCameraReset({
    commandsManager,
    getEnabledElement,
  });
  ...
  function elementEnabledHandler(evt) {
    ...
    element.addEventListener(EVENTS.CAMERA_RESET, resetCrosshairsOnCameraReset);
