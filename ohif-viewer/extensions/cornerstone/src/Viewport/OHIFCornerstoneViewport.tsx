...
const OHIFCornerstoneViewport = React.memo(props => {
  ...
  useEffect(() => {
    ...
    cornerstoneViewportService.enableViewport(viewportId, viewportElement);
    ...
    return () => {
      ...
      // A new study can reuse this viewport id before the old route's cleanup
      // runs.  Only clean up the viewport owned by this component instance.
      if (!viewportInfo || viewportInfo.getElement() !== viewportElement) {
      ...
      cornerstoneViewportService.disableElement(viewportId, viewportElement);
  ...
  useEffect(() => {
    ...
    const loadViewportData = async () => {
      ...
      // Volume creation may finish after navigation.  Do not let an old study
      // overwrite a newly mounted viewport that happens to have the same id.
      const currentViewportInfo = cornerstoneViewportService.getViewportInfo(viewportId);

      // Cornerstone3D V3: After a mode exit/enter cycle (e.g., switching
      // studies), cornerstoneViewportService.destroy() clears viewportsById.
      // The component stays mounted (key={viewportId} unchanged), so the
      // mount-only useEffect with enableViewport() never re-runs.
      // Re-register the viewport here if it was wiped by destroy().
      let availableViewportInfo = currentViewportInfo;
      if (!availableViewportInfo) {
        cornerstoneViewportService.enableViewport(viewportId, viewportElement);
        availableViewportInfo = cornerstoneViewportService.getViewportInfo(viewportId);
      }

      if (
        cancelled ||
        (!currentViewportInfo && !availableViewportInfo) ||
        (currentViewportInfo && currentViewportInfo?.getElement() !== viewportElement)
      ) {
        return;
      }
    ...
    loadViewportData().catch(e => {
      console.warn(
        'OHIFCornerstoneViewport::loadViewportData failed for viewport',
        viewportId,
        e
      );
    });
...
function _jumpToMeasurement(
  ...
  if (enabledElement) {
    ...
    csUtils.jumpToSlice(targetElement, {
