...
export function WindowLevelActionMenu({
...
}: WindowLevelActionMenuProps): ReactElement | null {
  const {
    colormaps,
  ...
  useEffect(() => {
    if (!viewport || !colorbarService.hasColorbar(viewportId)) {
      return;
    }
  ...
  // During study navigation the action-corner provider can render once between
  // destruction of the old Cornerstone3D viewport and registration of the new
  // one. That is a normal transition, so render no menu for that frame.
  if (!viewportInfo || !viewport) {
    return null;
  }
