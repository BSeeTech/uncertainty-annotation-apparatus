...
function commandsModule({
  ...
  const actions = {
    ...
    setSourceViewportForReferenceLinesTool: ({ viewportId }) => {
      ...
      // ReferenceLines is only added to tool groups that use it (e.g. 'mpr').
      // Skip silently for tool groups that don't have it (e.g. 'default').
    ...
    resetCrosshairs: ({ viewportId }) => {
      ...
      const getCrosshairInstances = toolGroupId => {
        ...
        if (!toolGroup?.hasTool('Crosshairs')) {
          return; // Crosshairs is only added to MPR tool groups
        }
