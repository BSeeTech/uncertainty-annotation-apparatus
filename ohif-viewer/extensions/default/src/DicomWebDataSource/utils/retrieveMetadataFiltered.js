  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.all(promises).then(results => {
      ...
      results.forEach(result => {
        // The sync loader (enableStudyLazyLoad=false) returns a plain array
        // of instances. The async loader returns { preLoadData, promises }.
        if (Array.isArray(result)) {
          aggregatedResult.preLoadData = aggregatedResult.preLoadData.concat(result);
        } else {
          aggregatedResult.preLoadData = aggregatedResult.preLoadData.concat(
            result.preLoadData || []
          );
          aggregatedResult.promises = aggregatedResult.promises.concat(
            result.promises || []
          );
        }
      });
