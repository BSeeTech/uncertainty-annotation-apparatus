...
function createDicomWebApi(dicomWebConfig, servicesManager) {
  ...
  const implementation = {
    ...
    _retrieveSeriesMetadataSync: async (
      ...
      // retrieveStudyMetadata can return either a plain array (non-filtered path)
      // or { preLoadData, promises } (filtered path via retrieveMetadataFiltered).
      // Handle both to avoid "data.map is not a function" when filters are applied.
      const rawInstances = Array.isArray(data) ? data : (data?.preLoadData || []);
      const naturalizedInstancesMetadata = rawInstances.map(naturalizeDataset);
