...
function OHIFCornerstoneSEGViewport(props) {
  ...
  let referencedDisplaySetMetadata = null;
  try {
    referencedDisplaySet = segDisplaySet.getReferenceDisplaySet();
    if (referencedDisplaySet) {
      referencedDisplaySetMetadata = _getReferencedDisplaySetMetadata(
    ...
    console.warn(
      'OHIFCornerstoneSEGViewport::could not get reference display set for SEG',
      e
    );
  }
  ...
  const getCornerstoneViewport = useCallback(() => {
    ...
    const ref = referencedDisplaySetRef.current;
    const referencedDisplaySet = ref?.displaySet;

    // If there's no referenced display set, render just the SEG display set
    // or an empty viewport to avoid crashes down the pipeline.
    const displaySetsToRender = referencedDisplaySet
      ? [referencedDisplaySet, segDisplaySet]
      : [segDisplaySet];
    ...
    return (
      <Component
        {...props}
        displaySets={displaySetsToRender}
        viewportOptions={{
          viewportType: 'volume',
  ...
  const metadata = referencedDisplaySetRef.current?.metadata || {};
  const {
    PatientID,
    PatientName,
    PatientSex,
